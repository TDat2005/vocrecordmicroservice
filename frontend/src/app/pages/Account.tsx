...
export function Account() {
  ...
  useEffect(() => {
    ...
    if (userData.customer_id) {
       // Profile
       fetch(API.users.getProfile(userData.customer_id))
         .then(res => res.json())
       ...
       // Wishlist
       fetch(API.users.getWishlist(userData.customer_id))
         .then(res => res.json())
  ...
  const fetchOrdersData = (custId: any) => {
       fetch(`${API.orders.list}?customer_id=${custId}`)
         .then(res => res.json())
  ...
  const handleSaveProfile = () => {
    ...
    fetch(API.users.updateProfile(user.customer_id), {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
  ...
  const handleRemoveWishlist = (productId: number) => {
    if(!user || !user.customer_id) return;
    fetch(API.users.removeWishlist, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
                    ...
                    <div key={item.id} className="border-2 border-black hover:shadow-lg transition-shadow group flex flex-col">
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <div className="mt-auto flex gap-2">
                          <button onClick={() => addToCart({ id: item.id, title: item.title, artist: item.artist, price: item.price, image: item.image, stock: item.stock || 99 })} className="flex-1 px-3 py-2 bg-black text-white font-bold text-xs uppercase hover:bg-gray-800 transition-colors">
                            VÀO GIỎ
                          </button>
