...
export function Turntables() {
    ...
    fetch('http://localhost/clonevocrecord/api/products.php?action=list')
      ...
      .then(data => {
        if (data.success && data.data) {
          // Chỉ lấy sản phẩm thuộc loại Mâm đĩa
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero Section */}
      <div className="bg-blue-600 text-white border-b-2 border-black py-16">
        <div className="container mx-auto px-4">
          ...
          <p className="text-xl lg:text-2xl font-bold max-w-2xl uppercase">
            Khám phá bộ sưu tập mâm đĩa tiêu chuẩn. Tối ưu trải nghiệm analog.
          </p>
        </div>
      </div>
          ...
          <main className="flex-1">
            ...
            {filteredRecords.length === 0 ? (
              <div className="text-center py-16 border-2 border-black bg-white">
                <p className="text-black font-bold uppercase">Chưa có sản phẩm Mâm đĩa nào được cập nhật lên hệ thống.</p>
              </div>
            ) : (
                ...
                {filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <div className="p-4 flex-1 flex flex-col">
                      <h3 className="font-bold uppercase text-lg line-clamp-1 mb-1">{record.title}</h3>
                      <p className="text-gray-700 font-bold uppercase text-sm mb-4 line-clamp-1">{record.artist}</p>
                      <div className="mt-auto">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-black font-bold text-xl">{formatPrice(record.price)}</span>
                          <span className="text-xs text-black border-2 border-black px-2 py-1 font-bold">{record.year}</span>
                        </div>
                        <div className="mt-2 h-8">
                          {record.stock > 0 ? (
                            <span className="bg-green-400 border-2 border-black px-2 py-1 text-xs font-bold uppercase text-black inline-block">Còn hàng</span>
                          ) : (
                            <span className="bg-red-600 border-2 border-black px-2 py-1 text-xs font-bold uppercase text-white inline-block">Hết hàng</span>
                          )}
                        </div>
                      </div>
                    </div>
